...
class SourceApiMuellabfuhrde extends BaseSource {
    ...
    async validate() {
        ...
        if (!mandator || !cityId) {
            this.adapter.log.warn('[api-muellabfuhrde] configuration incomplete');
            return false;
        }

        if (!streetId && (await this.isStreetRequired(mandator, cityId))) {
            this.adapter.log.warn('[api-muellabfuhrde] configuration incomplete - street selection required');
            return false;
        }
    ...
    async getPickupDates() {
        ...
        try {
            ...
            return (Array.isArray(pickups) ? pickups : [])
                .filter(p => p?.date && p?.fraction?.name)
                .map(p => ({
                    date: p.date,
            ...
            this.adapter.log.error(`[api-muellabfuhrde] unable to load pickups: ${err}`);
    ...
    async getApiCities(mandatorId) {
        ...
        return this.sortByName(cities.children || []);
    }
    ...
    async getApiStreets(mandatorId, cityId) {
        ...
        return this.sortByName(streets.children);
    }

    async isStreetRequired(mandatorId, cityId) {
        const streets = await this.getApiStreets(mandatorId, cityId);
        return streets.some(street => `${street.id}` !== `${cityId}`);
    }

    async getApiTypes(mandatorId, cityId, _districtId, streetId, _houseNumber) {
        ...
        (Array.isArray(pickups) ? pickups : [])
            .filter(p => p?.fraction?.id && p?.fraction?.name)
            .forEach(p => {
                types[p.fraction.id] = {
                    id: p.fraction.id,
                    name: p.fraction.name,
    ...
    sortByName(items) {
        return items.sort((a, b) => `${a.name}`.localeCompare(`${b.name}`, 'de', { sensitivity: 'base' }));
    }
}
